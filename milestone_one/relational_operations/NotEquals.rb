...
class NotEquals
    ...
    # constructs an unevaluated form of the relational not equals expression
    def initialize(first_operand, second_operand, start_index, end_index)
        ...
        if ((!second_operand.is_a? IntegerPrimitive) && (!second_operand.is_a? FloatPrimitive) &&
            ...
            puts "  Must provide either IntegerPrimtive, FloatPrimitive, StringPrimitive,
            or BooleanPrimitive type."
            return
        end
    ...
    # evaluates the relational not equals expression
    ...
    # generate string representation of the relational not equals expression
    def to_s
...
compare = NotEquals.new(intOne, intTwo)
puts compare
puts compare.evaluate(environment)

compare = NotEquals.new(intOne, intOne)
puts compare
puts compare.evaluate(environment)

compare = NotEquals.new(floatOne, floatTwo)
puts compare
puts compare.evaluate(environment)

compare = NotEquals.new(floatTwo, floatTwo)
puts compare
puts compare.evaluate(environment)

compare = NotEquals.new(t, f)
puts compare
puts compare.evaluate(environment)

compare = NotEquals.new(f, f)
puts compare
puts compare.evaluate(environment)

compare = NotEquals.new(stringOne, stringTwo)
puts compare
puts compare.evaluate(environment)

compare = NotEquals.new(stringTwo, stringTwo)
puts compare
puts compare.evaluate(environment)

# error
error = "test"
compare = NotEquals.new(error, stringOne)

# TypeError
compare = NotEquals.new(intOne, f)
puts compare
puts compare.evaluate(environment)
=end